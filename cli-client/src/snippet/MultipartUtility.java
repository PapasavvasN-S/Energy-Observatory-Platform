...
public class MultipartUtility {
    ...
    public MultipartUtility(String requestURL, String charset) throws IOException {
        ...
        if (file.exists()) {
            ...
            String token;
            token = br.readLine();
            httpConn.setRequestProperty("X-OBSERVATORY-AUTH", token);
            br.close();
        }
        httpConn.setRequestProperty("Content-Type", "multipart/form-data; boundary=" + boundary);
        httpConn.setRequestProperty("fileName", "import_this.csv");
    ...
    public List<String> finish() throws IOException {
        ...
        if (status == HttpURLConnection.HTTP_OK) {
            ...
            response.add("Error 400\nBad Request");
        else if (httpConn.getResponseCode() == 401)
            response.add("Error 401\nNot Authorized");
        else if (httpConn.getResponseCode() == 402)
            response.add("Error 402\nOut or quota");
        else if (httpConn.getResponseCode() == 403)
            response.add("Error 403\nNo Data");
